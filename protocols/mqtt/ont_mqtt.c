...
static int update_access_time(ont_device_t *device);
...
static int decode_variable_length(const char *buf,size_t* value,size_t*read_size);

static int ont_handle_process_err(ont_device_t *device,int err);
...

{
    ...
    ONT_LOG_DEBUG2("recv publish packet topic = %s, payload_len = %u",ctype_topic_name,payload_len);
	...
	const char *payload   ,size_t payload_len)
{
    ...
    size_t sys_cmd_topic_len = strlen(SYS_CMD_REQ_PLATFORM_CMD);
    ...
    memcpy(cmd->req, payload,payload_len);
...
int ont_send_mqtt_ack(ont_device_t *device,uint16_t packet_id)
{
    ont_mqtt_device_t *mqtt_instance = DEV2MQTT(device);
    int err = 0;
    ont_pkt_formatter_reset(device->formatter,0);

    err = ont_serialize_mqtt_publish_ack(device, packet_id);
    CHECK_ONT_ERR_OK_RETURN(ONT_ERR_OK, err)
	...
	size_t *packet_read_size)
{
    ...
    topic_len_tmp = (*(buf + (*fix_header_bytes)) << 8) | *(buf+(*fix_header_bytes)+1);

    /*
       payload 的长度为remain_length的长度-topic_len_tmp(uint16_t) - topic_name的长度
       */

    packet.payload_.len = *remain_length - topic_len_tmp - sizeof(uint16_t) - sizeof(uint16_t) /* packetid */;
    ...
    *packet_read_size = *fix_header_bytes + *remain_length;
    ...
    /*  send ack */
    err = ont_send_mqtt_ack(device,packet.packet_identifier_.value);
    CHECK_ONT_ERR_OK_RETURN(ONT_ERR_OK, err);
	...
	size_t *packet_read_size)
{
    ...
    packet.payload_.len = *remain_length - topic_len_tmp - sizeof(uint16_t);
